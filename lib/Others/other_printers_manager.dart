...
class OtherPrinterManager {
  ...
  Future<void> startScan() async {
    try {
      ...
      if (Platform.isAndroid) {
        _devicesstream.add(((await FlutterBluePlus.systemDevices) as List<BluetoothDevice>)
            .map((e) => Printer(
  ...
  void getPrinters({
    ...
    if (connectionTypes.contains(ConnectionType.BLE)) {
      ...
      await FlutterBluePlus.stopScan();
      await FlutterBluePlus.startScan();
      final systemDevices = ((await FlutterBluePlus.systemDevices) as List<BluetoothDevice>)
          .map((e) => Printer(
                address: e.remoteId.str,
                name: e.platformName,
                connectionType: ConnectionType.BLE,
